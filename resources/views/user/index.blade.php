...
var table = $('#content-data').DataTable({
    ...
    displayStart: _start,
    ajax: {
        url: "{{ route('panel.user.get-users') }}",
        type: 'POST',
        data: function(d) {
            d.filters = {
                q: $('#filter').val()
    ...
    columns: [
        ...
        {data: 'key', className: 'onclick-row', width: 100},
        {data: 'name', className: 'onclick-row'},
        {data: 'route', className: 'onclick-row'},
        {data: 'icon', className: 'onclick-row'},
    ...
    drawCallback: function() {
        ...
        $('.editElement').click(function(e){
            ...
            axios.get("{{ route('panel.user.get-user') }}/" + _id)
        ...
        $(".changeState").click(function(e){
            ...
            axios.post("{{ route('panel.user.change-state-user') }}",{
                id : $(this).data('id')
            })
            .then(function (response){
                $('#content-data').DataTable().ajax.reload(null, false);
...
$('#content-data tbody').on('dblclick', '.onclick-row', function (e) {
    ...
    axios.get("{{ route('panel.user.get-users') }}/" + data.id)
    .then(function (response){
        $('#content-modal').html(response.data.html);
    })
    .catch(function(error){
...
$('.new-registry').click(function(e){
    ...
    axios.get("{{ route('panel.user.get-user') }}/")
    .then(function (response){
        $('#content-modal').html(response.data.html);
    })
    .catch(function(error){

...
var table = $('#content-data').DataTable({
    ...
    displayStart: _start,
    ajax: {
        url: "{{ route('panel.institution.get-institutions') }}",
        type: 'POST',
        data: function(d) {
            d.filters = {
                q: $('#filter').val()
...
$('.new-registry').click(function(e){
    ...
    axios.get("{{ route('panel.institution.get-institution') }}/")
    .then(function (response){
        $('#content-body').html(response.data.html);
        window.history.pushState("data","Title","{{ route('panel.institution.get-institution') }}/");
    })
    .catch(function(error){
        toast.fire({
            text: 'No ha podido realizar la operación, error obteniendo el registro',
